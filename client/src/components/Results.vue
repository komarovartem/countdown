<template>
    <div class="results">
        ...
        <div>Players had these words</div>
        <div v-for="(words, index) in playersLetters" :key="index" v-if="words.length">
            <b>{{ words.length }}</b> players had <b>{{ index.slice(0, -1) }}</b> letters word
        </div>
        <div>
            {{ getLazyPlayers }}
        </div>
